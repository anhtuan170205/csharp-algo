...
The first line contains integer `n` (`1 ≤ n ≤ 2000`) — the number of tasks.

The second line contains `n` integers `h1, h2, ..., hn` (`1 ≤ hi ≤ 2000`), where `hi` is the difficulty of the `i`-th task. The larger number `hi` is, the more difficult the `i`-th task is.

## Output Format

In the first line print `"YES"` (without the quotes), if it is possible to come up with three distinct plans of doing the tasks. Otherwise print in the first line `"NO"` (without the quotes).

If three desired plans do exist, print in the second line `n` distinct integers that represent the numbers of the tasks in the order they are done according to the first plan. In the third and fourth line print two remaining plans in the same form.
